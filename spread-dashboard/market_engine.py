...
class MarketEngine:
    ...
    async def refresh_alpha(self):
        headers = {"User-Agent": "spread-dashboard/3.0", "Accept": "application/json"}
    ...
    async def refresh_bstocks(self):
        ...
        async with httpx.AsyncClient(headers=headers) as c:
            while True:
                ...
                rows = []
                for x in (listing.get("data") or []) if isinstance(listing, dict) else []:
                    ticker = str(x.get("ticker") or "").upper()
                    addr, chain = x.get("contractAddress"), str(x.get("chainId") or "")
                    if ticker not in BSTOCK_TICKERS or not addr: continue
                    d = await self.http_json(c, BSTOCK_PRICE_URL, {"chainId": chain, "contractAddress": addr})
                    try: price = float(d["data"]["tokenInfo"]["price"])
                    except Exception: price = None
                    rows.append({"ticker": ticker, "symbol": x.get("symbol"), "price": price, "chainId": chain, "ts": int(time.time()*1000)})
                async with self.lock:
                    if rows: self.bstocks = rows
        ...
        while True:
            try:
                async with websockets.connect(BYBIT_WS, ping_interval=20, ping_timeout=10, max_size=8*1024*1024) as ws:
                    ...
                    for i in range(0, len(args), 10): await ws.send(json.dumps({"op":"subscribe","args":args[i:i+10]}))
                    async for raw in ws:
                        m = json.loads(raw)
            ...
            try:
                async with websockets.connect(OKX_WS, ping_interval=20, ping_timeout=10, max_size=8*1024*1024) as ws:
                    ...
                    for i in range(0, len(args), 100):
                        if args[i:i+100]: await ws.send(json.dumps({"op":"subscribe","args":args[i:i+100]}))
                    async for raw in ws:
                        m = json.loads(raw)
    ...
    async def snapshot(self):
        ...
        rows = []
        for a in alpha:
            key = norm(a["coin"] + "USDT")
            venues = {name: ext[name].get(key) for name in ext}
            rows.append({"symbol":a["alpha_id"],"coin":a["coin"],"alpha":a["price"],"alpha_ts":a["ts"],"venues":venues,"spreads":{name:spread_pct(a["price"],q.get("last") if q else None) for name,q in venues.items()}})
        return {"updated_at":int(time.time()*1000),"alpha":rows,"bstocks":stocks,"connections":connections,"mode":"server-websocket"}
