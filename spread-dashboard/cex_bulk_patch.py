...
def install_cex_bulk(engine):
    ...
    async def bybit_fetch(self):
        alpha = await self._cex_coins()
        wanted = {v for r in alpha for v in _base_aliases(r)}
        ...
        for d in (payload.get("result") or {}).get("list") or []:
            ...
            try:
                ...
                if bid > 0 and ask > 0:
                    _store_aliases(out, s, {"bid": bid, "ask": ask, "last": float(d.get("lastPrice") or (bid + ask) / 2), "ts": int(payload.get("time") or time.time() * 1000)}, alpha)
            except Exception:
                pass
        return out
    ...
    async def gate_fetch(self):
        ...
        for d in payload if isinstance(payload, list) else []:
            ...
            try:
                ...
                if bid > 0 and ask > 0:
                    _store_aliases(out, s.replace("_", ""), {"bid": bid, "ask": ask, "last": float(d.get("last") or (bid + ask) / 2), "ts": int(time.time() * 1000)}, alpha)
            except Exception:
                pass
        return out

    async def start(self):
        # Alpha refresh is owned by sitecustomize + alpha_ws_patch.  Do not
        # call the removed seed_missing_alpha() task here: the CEX bulk patch
        # must never be able to crash application startup.
        self.tasks = [
            asyncio.create_task(self.refresh_alpha()),
            asyncio.create_task(self.refresh_alpha_depth()),
            asyncio.create_task(self.refresh_bstocks()),
        ]
        for _ in range(80):
        ...
        self.tasks += [
            ...
            asyncio.create_task(self._gate_loop()),
