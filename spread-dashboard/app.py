...
async def alpha_prices(client):
    # Binance Alpha public endpoints have changed over time. We first try the current
    # public market endpoint, then fall back to configured symbols on Binance spot.
    candidates = [
...
async def collect():
    async with httpx.AsyncClient(headers={"User-Agent": "spread-dashboard/1.0"}) as c:
        # bStocks: Binance spot pairs are used only for the Binance side; the dashboard
        # deliberately labels them as bStocks and allows the exact pair list to be edited.
        ...
        # If discovery fails, the UI remains usable and shows an explicit unavailable state.
        rows = []
        for sym in symbols:
...
:root{--bg:#080b12;--card:#111722;--line:#202938;--text:#eef2f7;--muted:#8e9aab;--green:#21d19a;--red:#ff6176;--accent:#f0b90b}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}main{max-width:1450px;margin:auto;padding:28px 22px}.top{display:flex;justify-content:space-between;align-items:end;margin-bottom:22px}.title{font-size:26px;font-weight:800}.sub{color:var(--muted);margin-top:4px}.status{font-size:12px;color:var(--muted)}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:22px}.card{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:15px}.label{color:var(--muted);font-size:12px}.value{font-size:20px;font-weight:750;margin-top:5px}.tabs{display:flex;gap:8px;margin:16px 0}.tab{border:1px solid var(--line);background:var(--card);color:var(--muted);padding:9px 14px;border-radius:10px;cursor:pointer}.tab.active{color:var(--text);border-color:#4a5568}.panel{background:var(--card);border:1px solid var(--line);border-radius:14px;overflow:auto}table{width:100%;border-collapse:collapse}th,td{text-align:right;padding:12px 14px;border-bottom:1px solid var(--line);white-space:nowrap}th:first-child,td:first-child{text-align:left}th{font-size:12px;color:var(--muted);font-weight:600;background:#0d121b;position:sticky;top:0}.green{color:var(--green)}.red{color:var(--red)}.neutral{color:var(--muted)}.note{color:var(--muted);font-size:12px;margin-top:10px}@media(max-width:900px){.grid{grid-template-columns:repeat(2,1fr)}.top{display:block}.status{margin-top:10px}}
</style></head><body><main><div class="top"><div><div class="title">跨交易所价差监控</div><div class="sub">Binance bStocks ↔ 参考资产 · Binance Alpha ↔ Gate / Bybit / OKX / Coinbase</div></div><div class="status" id="status">连接中…</div></div>
