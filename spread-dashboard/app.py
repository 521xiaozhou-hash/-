...
@app.get("/api/data")
async def api_data():
    d = await engine.snapshot(); d["alpha_count"] = len(d.get("alpha", [])); return d | {"version": version()}
...
</style></head><body><main><div class="top"><div><div class="title">实时跨市场价差监控</div><div class="sub">严格按「市场 A 买入 Ask → 市场 B 卖出 Bid」计算；行情由 Linux 服务器直接连接交易所</div></div><div><div class="status" id="status">行情引擎启动中…</div><div class="controls"><button class="btn" onclick="load()">↻ 立即更新</button><select id="interval" class="select" onchange="setTimer()"><option value="1" selected>1 秒</option><option value="2">2 秒</option><option value="5">5 秒</option><option value="10">10 秒</option></select><button class="btn" onclick="openBlacklist()">🚫 黑名单</button><button class="btn" onclick="openUpdate()">⚙ 程序更新</button></div></div></div><div class="grid"><div class="card"><div class="label">Alpha 币种</div><div class="value" id="n">—</div></div><div class="card"><div class="label">最大可执行价差</div><div class="value" id="max">—</div></div><div class="card"><div class="label">正向机会</div><div class="value" id="pos">—</div></div><div class="card"><div class="label">程序版本</div><div class="value" id="ver">—</div></div></div><div class="tabs"><button class="tab active" onclick="show('alpha',this)">Alpha ↔ CEX</button><button class="tab" onclick="show('stocks',this)">bStock ↔ Binance股票 ↔ Gate股票</button></div><div class="panel"><table id="table"></table></div><div class="note" id="note">价差列按「Alpha买→其他市场卖 / 其他市场买→Alpha卖」分别显示；每个方向取所有有盘口交易所中的最佳值。</div></main><div class="modal" id="modal"><div class="box"><h3>程序更新</h3><p id="programText">检查中…</p><div class="row"><input class="input" id="token" placeholder="输入服务器更新密钥" type="password"></div><div class="controls"><button class="btn" onclick="checkProgram()">检查新版本</button><button class="btn" onclick="doProgramUpdate()">立即更新</button><button class="btn" onclick="closeModal()">关闭</button></div></div></div><div class="modal" id="blackmodal"><div class="box"><h3>币种黑名单</h3><p>每行一个，例如 SXT、KAT、ZKC。保存后服务器立即过滤。</p><textarea class="textarea" id="blacklist"></textarea><div class="controls"><button class="btn" onclick="saveBlacklist()">保存</button><button class="btn" onclick="closeBlacklist()">关闭</button></div></div></div><script>
...
function directional(x){let ab=x.alpha, first=[], second=[];for(let [name,q] of Object.entries(x.venues||{})){let s1=spread(ab?.ask,q?.bid);let s2=spread(q?.ask,ab?.bid);if(s1!=null)first.push({s:s1,n:name});if(s2!=null)second.push({s:s2,n:name})}first.sort((a,b)=>b.s-a.s);second.sort((a,b)=>b.s-a.s);return {a:first[0]||null,b:second[0]||null}}
function op2(x){let d=directional(x);let left=d.a?`<span class="${cls(d.a.s)}">${pct(d.a.s)}</span><br><span class="muted" style="font-size:11px">Alpha买 → ${d.a.n}卖</span>`:'—';let right=d.b?`<span class="${cls(d.b.s)}">${pct(d.b.s)}</span><br><span class="muted" style="font-size:11px">${d.b.n}买 → Alpha卖</span>`:'—';return `<div>${left}<span class="muted"> / </span>${right}</div>`}
function stockOp(x){let m=x.markets||{},names=Object.keys(m),left=[],right=[];for(let i=0;i<names.length;i++)for(let j=0;j<names.length;j++)if(i!==j){let s=spread(m[names[i]]?.ask,m[names[j]]?.bid);if(s!=null){if(names[i]==='bStock')left.push({s,n:names[j]});else if(names[j]==='bStock')right.push({s,n:names[i]});}}left.sort((a,b)=>b.s-a.s);right.sort((a,b)=>b.s-a.s);let l=left[0]?`<span class="${cls(left[0].s)}">${pct(left[0].s)}</span><br><span class="muted" style="font-size:11px">bStock买 → ${left[0].n}卖</span>`:'—';let r=right[0]?`<span class="${cls(right[0].s)}">${pct(right[0].s)}</span><br><span class="muted" style="font-size:11px">${right[0].n}买 → bStock卖</span>`:'—';return `${l}<span class="muted"> / </span>${r}`}
function render(){let t=document.getElementById('table');if(mode==='stocks'){t.innerHTML='<thead><tr><th>股票</th><th>bStock<br>Bid / Ask</th><th>Binance 股票<br>Bid / Ask</th><th>Gate 股票<br>Bid / Ask</th><th>最佳可执行价差<br>bStock买→卖出 / 买入→bStock卖</th></tr></thead><tbody>'+data.bstocks.map(x=>{let b=x.markets?.['bStock'],bn=x.markets?.['Binance Stock'],g=x.markets?.['Gate Stock'];return `<tr><td><b>${x.underlying}</b><br><span class="muted">${x.ticker}</span></td><td>${fmt(b?.bid)} / ${fmt(b?.ask)}</td><td>${bn?fmt(bn.bid)+' / '+fmt(bn.ask):'未配置'}</td><td>${fmt(g?.bid)} / ${fmt(g?.ask)}</td><td>${stockOp(x)}</td></tr>`}).join('')+'</tbody>';return}t.innerHTML='<thead><tr><th>Alpha</th><th>Alpha<br>Bid / Ask</th><th>Bybit<br>Bid / Ask</th><th>Gate<br>Bid / Ask</th><th>OKX<br>Bid / Ask</th><th>Coinbase<br>Bid / Ask</th><th>最佳可执行价差<br>Alpha买→其他卖 / 其他买→Alpha卖</th></tr></thead><tbody>'+data.alpha.map(x=>`<tr><td><b>${x.coin}</b><br><span class="muted">${x.symbol}</span></td><td>${fmt(x.alpha?.bid)} / ${fmt(x.alpha?.ask)}</td>`+['Bybit','Gate','OKX','Coinbase'].map(v=>{let q=x.venues[v];return `<td>${fmt(q?.bid)} / ${fmt(q?.ask)}</td>`}).join('')+`<td>${op2(x)}</td></tr>`).join('')+'</tbody>'}
function conn(c){return Object.entries(c||{}).map(([k,v])=>`${k}:${v}`).join(' · ')}
async function load(){if(busy)return;busy=true;try{let r=await fetch('/api/data',{cache:'no-store'});data=await r.json();let vals=[];data.alpha.forEach(x=>{let d=directional(x);if(d.a)vals.push(d.a.s);if(d.b)vals.push(d.b.s)});data.bstocks.forEach(x=>{let m=x.markets||{},names=Object.keys(m);for(let i=0;i<names.length;i++)for(let j=0;j<names.length;j++)if(i!==j){let s=spread(m[names[i]]?.ask,m[names[j]]?.bid);if(s!=null)vals.push(s)}});document.getElementById('n').textContent=data.alpha_count??data.alpha.length;document.getElementById('max').textContent=vals.length?Math.max(...vals).toFixed(3)+'%':'—';document.getElementById('pos').textContent=vals.filter(x=>x>0).length;document.getElementById('ver').textContent=data.version||'—';let ad=data.alpha_diagnostics||{};document.getElementById('status').textContent='服务器行情 · '+new Date(data.updated_at).toLocaleTimeString()+' · '+conn(data.connections);document.getElementById('note').textContent=`Alpha：Token List ${ad.token_list||0} · Exchange Info ${ad.exchange_info||0} · 当前可交易 ${ad.active||0} · 页面显示 ${data.alpha_count||0} · ${ad.error||'正常'}`;render()}catch(e){document.getElementById('status').textContent='服务器行情接口不可用'}finally{busy=false}}
